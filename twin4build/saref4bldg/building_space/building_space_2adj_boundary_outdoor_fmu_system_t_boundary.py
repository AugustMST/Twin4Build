...
def get_signature_pattern():
    ...
    node12 = Node(cls=base.Sensor, id="<n<SUB>13</SUB>(Sensor)>")
    node13 = Node(cls=base.Temperature, id="<n<SUB>14</SUB>(Temperature)>")

    node14 = Node(cls=base.Sensor, id="<n<SUB>15</SUB>(Sensor)>")

    sp = SignaturePattern(ownedBy="BuildingSpace2AdjBoundaryOutdoorFMUSystemTBoundary", priority=500)
    ...
    sp.add_edge(Exact(object=node2, subject=node14, predicate="contains"))
    sp.add_edge(Exact(object=node14, subject=node13, predicate="observes"))

    sp.add_edge(Exact(object=node11, subject=node12, predicate="contains"))
    sp.add_edge(Exact(object=node11, subject=node13, predicate="hasProperty"))
    sp.add_edge(Exact(object=node12, subject=node13, predicate="observes"))

    sp.add_input("T_boundary", node14, "measuredValue")
...
def get_signature_pattern_test():
    """
    Get the signature pattern of the FMU component.
    ...
    sp = SignaturePattern(ownedBy="BuildingSpace2AdjBoundaryOutdoorFMUSystemTBoundary_V2", priority=600)
    ...
    sp.add_input("T_boundary", node12, "measuredValue")
    sp.add_input("airFlowRate", node0)
    sp.add_input("waterFlowRate", node3)
    sp.add_input("numberOfPeople", node5, "scheduleValue")
    sp.add_input("outdoorTemperature", node6, "outdoorTemperature")
...
class BuildingSpace2AdjBoundaryOutdoorFMUSystemTBoundary(FMUComponent, base.BuildingSpace, base.SpaceHeater):
    """
    ...
    """
    sp = [get_signature_pattern(), get_signature_pattern_test()]
    def __init__(self,
                C_supply=None,
                C_wall=None,
                C_air=None,
